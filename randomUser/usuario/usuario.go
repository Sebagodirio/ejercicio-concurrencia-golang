...
func generarEmail(nombre, apellido string) string {
...
func EscribirArchivo(file *os.File, cantidad int) {
	...
	for i := 0; i < cantidad; i++ {
		...
		datos, _ := json.Marshal(user)            //Pasando a json
		_, err := file.WriteString(string(datos)) //Escribiendo el archivo
		if err != nil {
